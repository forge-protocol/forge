...
export async function generateSdkV2(config: SdkConfig, idl: any): Promise<void> {
...
`;
    writeFileSync(join(outputDir, 'src', 'rpc.ts'), rpcContent);

    // Generate instructions.ts
    const instructionsContent = generateInstructionsV2(programName, programId, idl);
    writeFileSync(join(outputDir, 'src', 'instructions.ts'), instructionsContent);

    // Generate accounts.ts
    const accountsContent = generateAccountsV2(programName, idl);
    writeFileSync(join(outputDir, 'src', 'accounts.ts'), accountsContent);

    // Generate pdas.ts
    const pdasContent = generatePdasV2(programName, programId, idl);
    writeFileSync(join(outputDir, 'src', 'pdas.ts'), pdasContent);
}

function generateInstructionsV2(programName: string, programId: string, idl: any): string {
    let content = `import { 
  address, 
...
function generateAccountsV2(programName: string, idl: any): string {
...
function generatePdasV2(programName: string, programId: string, idl: any): string {
...
function mapIdlTypeToTsV2(type: any): string {
...
function getEncoderForTypeV2(type: any): string {
    if (typeof type === 'string') {
        switch (type) {
