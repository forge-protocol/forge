...
import { ForgeConfig, ProgramConfig, DeploymentConfig, ValidationResult, ValidationError, ValidationWarning } from '../types';

/**
 * Validates a FORGE configuration object
 */
export function validateForgeConfig(config: unknown): ValidationResult {
 ...
 */
export function validateProgramConfig(config: unknown): ValidationResult {
 ...
 */
export function validateForgeProject(config: ForgeConfig): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationWarning[] = [];
  ...
  config.programs.forEach((program, index) => {
    ...
    programValidation.errors.forEach((error: ValidationError) => {
      errors.push({
        ...error,
        field: `programs[${index}].${error.field}`
      });
    });
    programValidation.warnings.forEach((warning: ValidationWarning) => {
      warnings.push({
        ...warning,
    ...
    if (!programIdValidation.isValid) {
      programIdValidation.errors.forEach((error: ValidationError) => {
        errors.push({
          ...error,
    ...
    programIdValidation.warnings.forEach((warning: ValidationWarning) => {
      warnings.push({
        ...warning,
        field: `programs[${index}].id`
      });
